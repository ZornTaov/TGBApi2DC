...
namespace TGBApi2DC
{
    class TGBApi2DC
    {
        ...
        static List<TypeFields> getTypeGroup2(string typeName, List<Types> typesList, out List<Types> typesListOut)
        {
            typesListOut = (from type in typesList
        ...
        public static void GenerateContracts(rootJson result)
        {
            ...
            /// PassportElementError setup
            List<Types> ChatMemberTypes;
            List<TypeFields> ChatMemberCommonFields = getTypeGroup("ChatMember", result.types, out ChatMemberTypes);

            /// Keyboard setup
            List<Types> keyboardsTypes;
            getTypeGroup2("KeyboardMarkup", result.types, out keyboardsTypes);
            ...
            foreach (Types type in result.types)
            {
                ...
                using (StreamWriter sw = File.CreateText(String.Format(@".\{0}\{1}.cs", fieldPath, type.name)))
                {
                    sw.WriteLine(DCStringFormats.LICENSE, DateTime.Now.Year);
                    if (type.name.Equals("InputFile"))
                    {
                        sw.WriteLine(DCStringFormats.HTTP_HEADER);
                    }
                    if (type.name.Contains("KeyboardMarkup"))
                    {
                        sw.WriteLine(DCStringFormats.LIST_HEADER);
                    }
                    sw.WriteLine(DCStringFormats.FILE_HEADER);
                    sw.WriteLine("{");
                    if (type.name.Contains("LoginUrl"))
                    {
                        type.description = Regex.Replace(type.description, @"\r\n?|\n", " ");
                    }
                    sw.WriteLine(DCStringFormats.SUMMARY, type.description, "\t");
                    /// edge cases:
                    {
                        /// InputFile
                        if (type.name == "InputFile")
                        {
                            sw.WriteLine(DCStringFormats.DC_HEADER_INHERITS, type.name, "StreamContent");
                        }
                        /// InputMedia
                        else if (type.name.StartsWith("InputMedia"))
                        {
                            switch (type.name)
                            {
                                ...
                                case "InputMedia":
                                    ...
                                    {
                                        sw.WriteLine(DCStringFormats.KNOWN_TYPE, item.name);
                                    }
                                    sw.WriteLine(DCStringFormats.DC_HEADER, type.name);
                                    type.fields = inputMediaCommonFields.Select(x => { x.description = x.description.Replace("photo", "media"); return x; }).ToList();
                                    type.fields[0].description = "Type of the result";
                                    break;
                                default:
                                    sw.WriteLine(DCStringFormats.DC_HEADER_INHERITS, type.name, "InputMedia");
                        ...
                        else if (type.name.StartsWith("InlineQueryResult"))
                        {
                            switch (type.name)
                            {
                                ...
                                case "InlineQueryResult":
                                    ...
                                    break;
                                default:
                                    sw.WriteLine(DCStringFormats.DC_HEADER_INHERITS, type.name, "InlineQueryResult");
                        ...
                        else if (type.name.StartsWith("PassportElementError"))
                        {
                            switch (type.name)
                            {
                                ...
                                case "PassportElementError":
                                    ...
                                    break;
                                default:
                                    sw.WriteLine(DCStringFormats.DC_HEADER_INHERITS, type.name, "PassportElementError");
                                    type.fields = type.fields.Where(x => !PassportElementErrorCommonFields.Any(y => x.name.Equals(y.name))).ToList();
                                    break;
                            }
                        }
                        /// ChatMember
                        else if (type.name.StartsWith("ChatMember"))
                        {
                            switch (type.name)
                            {
                                /// commons are id, type
                                case "ChatMember":
                                    foreach (var item in ChatMemberTypes)
                                    {
                                        sw.WriteLine(DCStringFormats.KNOWN_TYPE, item.name);
                                    }
                                    sw.WriteLine(DCStringFormats.DC_HEADER, type.name);
                                    type.fields = ChatMemberCommonFields;
                                    break;
                                default:
                                    sw.WriteLine(DCStringFormats.DC_HEADER_INHERITS, type.name, "ChatMember");
                                    type.fields = type.fields.Where(x => !ChatMemberCommonFields.Any(y => x.name.Equals(y.name))).ToList();
                                    break;
                            }
                        }
                        ...
                        else if (type.name.Contains("KeyboardMarkup"))
                        {
                            switch (type.name)
                            {
                                ...
                                case "KeyboardMarkup":
                                    ...
                                    break;
                                default:
                                    sw.WriteLine(DCStringFormats.DC_HEADER_INHERITS, type.name, "KeyboardMarkup");
                                    break;
                            }
                        }
                        else
                        ...
                        if (type.name.Equals("InputFile"))
                        {
                            sw.WriteLine(DCStringFormats.SUMMARY, "Input File Stream", "\t\t");
                            sw.WriteLine(DCStringFormats.CONSTRUCTOR, type.name, "Stream", "stream");
                        }
                        ...
                        foreach (TypeFields field in type.fields)
                        {
                            ...
                            sw.WriteLine(DCStringFormats.SUMMARY, field.description.Replace(@"\n", " "), "\t\t");
                            if (field.hasHeader)
                            {
                                sw.WriteLine(DCStringFormats.DM_FIELD_HEADER,
                                    field.name,
                                    field.description.StartsWith("Optional.") ? DCStringFormats.DM_EMIT_DEFAULT : DCStringFormats.DM_REQUIRED
                                );
                            }
                            sw.WriteLine(field.altProperty != null ? DCStringFormats.DM_FIELD_PROPERTY : DCStringFormats.DM_FIELD,
                                field.name,
                                fieldType,
                                field.altProperty != null ? field.altProperty : field.defaultValue != null ? field.defaultValue : ""
                            );
                        }
                        sw.WriteLine("\t}");
                    }
